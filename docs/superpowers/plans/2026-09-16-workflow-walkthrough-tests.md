...
### Review wave, 2026-09-16 to 17

After the seven lanes, the owner asked for a two-axis code review, an architecture review, and further reviews with parallel Luna implementers. Findings and what was done, in commit order on this branch:

| Commit | What | Why |
| --- | --- | --- |
| `7b6710e` | Review findings: the harness drives real headless mouse and keyboard input (with a probe proving a click reaches a bound command); one exception-to-refusal mapping in the store command; one reference-document list in the Handoff view model; the command-surface doc says what the value means rather than which release exposes it. | Standards review: one hard violation, five judgement calls. Spec review: the harness only pretended to click. |
| `b444134` | One `CommandRunViewModel<TResponse>` behind Assess and Handoff; `RunState` replaces two enums; `FailureReason.Cancelled` in the contract; each view model owns `Reset()`. | Architecture review's three Strong candidates, which were one piece of work. |
| `6e23720` | The Handoff exports the retained invocation on screen and never runs its own Assessment; `--invocation` on the CLI; the Handoff button waits for a completed Assessment; the release plan's R2 exit test passes. | Spec review's worst finding, and R2's core; the retained storage it needed already existed. |
| `8f1828c` | README no longer names a nonexistent `apply` flag; the CLI reference regains every verb, the released/developer split, the opt-in variable and exit codes; `starter-prompt.md` documented and required by the reader; AGENTS.md explains the synthetic fixture; issues S1–S3. | Documentation review: seven mismatches. |
| `df6c76c` | A published Baseline is reported even when the machine store cannot record the project; a minimal parser child environment and a `--describe` surface check before the first long run; retained validation names its failing field; unknown ids become typed refusals; the release gate fails closed without leaking a sentinel. | Security review (one high) and quality review (one high, two medium). |
...
**Machine load is part of the evidence.** Two test hosts from another repository held 6 to 11 GB each for most of this wave. Under that load the cancel-Handoff walkthrough failed three times with an access-denied assigning the parser to its CPU job (the parser had exited first), then passed nine times in a row when the load eased; two spawned-CLI capture tests failed once with a Busy exit code and passed alone. A real-parser walkthrough that fails once under load is rerun before it is believed; the `WindowsCpuJob` assignment failure now says whether the process had exited.
